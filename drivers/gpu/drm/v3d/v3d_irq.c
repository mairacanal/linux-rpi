...
static void
v3d_irq_signal_fence(struct v3d_dev *v3d, enum v3d_queue queue,
		     void (*trace_irq)(struct drm_device *, uint64_t))
{
	struct v3d_queue_state *q = &v3d->queue[queue];
	struct v3d_fence *fence = to_v3d_fence(q->active_job->irq_fence);

	v3d_job_update_stats(q->active_job, queue);
